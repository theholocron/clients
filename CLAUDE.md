...
- **Running `holocron setup`**: `@theholocron/cli` and `@theholocron/holocron-plugin-github` are installed as devDependencies so setup is runnable locally. Use `GITHUB_TOKEN=$(gh auth token) pnpm exec holocron setup`. Workflow files, `.alexrc.json`, `.github/dependabot.yml`, and repo-policy changes (branch protection, security settings) are all applied in one pass.
