...
import type { JiraClientOptions, JiraIssueFields, JiraResolution, JiraTransition } from "./types.js";
...
export function transitions(options: JiraClientOptions) {
	...
	return {
		create(ticket: string, statusId: string, fields: JiraIssueFields = {}): Promise<void> {
			return request<void>(buildUrl(options, `/issue/${ticket}/transitions`), {
				method: "POST",
				headers,
				body: JSON.stringify({
