...
export function branches(rest: RestClient) {
	return {
		list: (projectId: string): Promise<NeonBranchesResponse> =>
			rest.request<NeonBranchesResponse>(
		...
		create: (
			...
			rest.request<NeonBranchResponse>(
				`/projects/${projectId}/branches`,
				{
					method: "POST",
					body: {
		...
		destroy: (projectId: string, branchId: string): Promise<void> =>
			rest.request<void>(
				`/projects/${projectId}/branches/${encodeURIComponent(branchId)}`,
				{
					method: "DELETE",
				},
			),
		...
		restore: (
			...
			rest.request<void>(
				`/projects/${projectId}/branches/${encodeURIComponent(branchId)}/restore`,
				{
					method: "POST",
