...
import type { JiraIssue, JiraIssueFields, JiraSearchQuery, JiraSearchResponse } from "./types.js";
...
export function issues(client: RestClient) {
	return {
		...
		getProperty(ticket: string, property: string): Promise<unknown> {
			return client.request<unknown>(`/issue/${ticket}/properties/${property}`);
		},

		search(query: JiraSearchQuery = {}): Promise<JiraSearchResponse> {
			const q: Record<string, string> = {};
			if (query.jql !== undefined) q["jql"] = query.jql;
			if (query.startAt !== undefined) q["startAt"] = String(query.startAt);
			if (query.maxResults !== undefined) q["maxResults"] = String(query.maxResults);
			if (query.fields !== undefined) q["fields"] = query.fields.join(",");
